<?php
	...
	function getNbPoiEnLien($commune, $voie, $titulaire)
	{
		include("connexionBddErp.php");
		$nbPoi = 0;
		
		$req = $bddErp->prepare("SELECT COUNT(*) nb FROM ag_poi WHERE (ft_titulaire_client = ? AND ft_titulaire_client != '' AND ft_titulaire_client IS NOT NULL) OR (ft_libelle_commune = ? AND ft_libelle_commune != '' AND ft_libelle_commune IS NOT NULL) OR (ft_libelle_commune = ? AND ft_libelle_de_voie = ? AND ft_libelle_commune != '' AND ft_libelle_de_voie != '' AND ft_libelle_commune IS NOT NULL AND ft_libelle_de_voie IS NOT NULL)");
		$req->execute(array($titulaire, $commune, $commune, $voie));
		if($data = $req->fetch())
		{
			$nbPoi = $data["nb"];
		}
	...
	function getCaffsEnLienAvecPoiByVoie($voie, $commune, $ui)
	{
		...
		$req = $bddErp->prepare("SELECT p.atr_caff_traitant_id, e.name_related, COUNT(p.ft_numero_de_voie) nb, e.ag_site_id 
		FROM ag_poi p 
		JOIN hr_employee e 
		ON p.atr_caff_traitant_id = e.id 
		WHERE p.ft_numero_de_voie = ?
		AND p.ft_numero_de_voie != ''
		AND p.ft_numero_de_voie IS NOT NULL
